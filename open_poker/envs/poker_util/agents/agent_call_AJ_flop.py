...
def make_flop_moves(player, current_gameboard, allowable_actions, code):
    ...
    sui_cnt = Counter(suits)
    is_high_pair_or_suits = False
    for val in val_cnt.values():
        if val >= 3:
            is_high_pair_or_suits = True
            break
    for val in sui_cnt.values():
        if val >= 3:
            is_high_pair_or_suits = True
            break
    ...
    if is_high_pair_or_suits and call in allowable_actions:
        return call, params
    # elif is_high_pair_or_suits and all_in in allowable_actions:
