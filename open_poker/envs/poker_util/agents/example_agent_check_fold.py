...
import numpy as np
"""
Strategy of this agent is to always check with amount of limit (don't want to take risk to all-in), or fold
"""
...
def make_pre_flop_moves(player, current_gameboard, allowable_actions, code):
    ...
    if player.big_blind and player.bet_amount_each_round == current_gameboard['board'].current_highest_bet:
        print(f'{player.player_name} is big blind, nothing need to be done')
...
def make_flop_moves(player, current_gameboard, allowable_actions, code):
    ...
    if check in allowable_actions:
        ...
        print('Error: somgthing go wrong')
        raise Exception
