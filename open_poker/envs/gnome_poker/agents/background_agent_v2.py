    ...
    if call in allowable_actions or raise_bet in allowable_actions:
        ...
        if suits[0] == suits[1]:  # hole cards are in same suit
            ...
            if (values[0], values[1]) in top_5_pair['suit'] or (values[1], values[0]) in top_5_pair['suit']:
                ...
                current_gameboard['hole_card_type_v2'] = ('top_1', 'suit')
                ...
                current_gameboard['hole_card_type_v2'] = ('top_2', 'suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                ...
                current_gameboard['hole_card_type_v2'] = ('top_3', 'suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                ...
                current_gameboard['hole_card_type_v2'] = ('top_4', 'suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                ...
                current_gameboard['hole_card_type_v2'] = ('top_5', 'suit')
            ...
            if (values[0], values[1]) in top_5_pair['un-suit'] or (values[1], values[0]) in top_5_pair['un-suit']:
                ...
                current_gameboard['hole_card_type_v2'] = ('top_1', 'un-suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                    if player.bet_amount_each_game + amount_to_raise < rank2profit_percent['top_1'] * (player.bet_amount_each_game + player.current_cash):
                        params['amount_to_raise'] = amount_to_raise
                ...
                print('I have top 6-14 pairs')
                current_gameboard['hole_card_type_v2'] = ('top_2', 'un-suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                    if player.bet_amount_each_game + amount_to_raise < rank2profit_percent['top_2'] * (player.bet_amount_each_game + player.current_cash):
                        params['amount_to_raise'] = amount_to_raise
                ...
                print('I have top 15-26 pairs')
                current_gameboard['hole_card_type_v2'] = ('top_3', 'un-suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                    if player.bet_amount_each_game + amount_to_raise < rank2profit_percent['top_3'] * (player.bet_amount_each_game + player.current_cash):
                        params['amount_to_raise'] = amount_to_raise
                ...
                print('I have top 27-40 pairs')
                current_gameboard['hole_card_type_v2'] = ('top_4', 'un-suit')
                if previous_bet > big_blind_force_bet and raise_bet in allowable_actions:  # re-raise again
                    amount_to_raise = 1.5 * previous_bet
                    if player.bet_amount_each_game + amount_to_raise < rank2profit_percent['top_4'] * (player.bet_amount_each_game + player.current_cash):
                        params['amount_to_raise'] = amount_to_raise
                ...
                print('I have top 40+ pairs')
                current_gameboard['hole_card_type_v2'] = ('top_5', 'un-suit')
                if player.bet_amount_each_game + previous_bet <= big_blind_force_bet:
                    return call, params

    print('I have bottom pairs')
    current_gameboard['hole_card_type_v2'] = ('bottom', 'un-suit') if suits[0] != suits[1] else ('bottom', 'suit')
...
def make_flop_moves(player, current_gameboard, allowable_actions, code):
    ...
    hole_card_type, hole_card_suit = current_gameboard['hole_card_type_v2']
...
def make_turn_moves(player, current_gameboard, allowable_actions, code):
    ...
    3. In this phase, the agent has 6 cards to consider and take actions appropriately. We say a hand is high cards
    if current hand has high chance to become straight or above. otherwise, because of community card, other agents also
    have high chance to get three of a kind, two pair, or others
        --> if straight, flush, or full house, call with amount no larger than
        --> if four of a kind, straight flush, or royal flush, all-in
        --> otherwise fold
    :param player:
    :param current_gameboard:
    :param allowable_actions:
    ...
    if all_in in allowable_actions:
        for su in suits:  # expect flush
            if suits.count(su) >= 4:
                return all_in, params
        for val in values:  # expect four of a kind
        ...
        for i in range(1, len(values_copy)):
            if values_copy[i] - 1 == values_copy[i-1]:
                ...
                cnt = 1
            max_cnt = max(cnt, max_cnt)
        if max_cnt >= 4:  # expect straight
            return all_in, params

    # if not meet condition above, means there are low chance to get straight or above
    if call in allowable_actions:
        for val in values:
            cur_cnt = values.count(val) >= 3
            if cur_cnt >= 3 and previous_bet <= 2 * minimum_bet and previous_bet <= player.current_cash:
                return call, params
            if cur_cnt >= 2 and previous_bet <= 1.5 * minimum_bet and previous_bet <= player.current_cash and \
                    (hole_card_type == 'top_1' or hole_card_type == 'top_2' or hole_card_type == 'top_3'):
                return call, params

    return fold, params  # if no other actions, just fold at this time
...
def make_river_moves(player, current_gameboard, allowable_actions, code):
    ...
    1. If all-in in previous round, the agent has to wait until the showdown
    2. If no others bet which mean previous bet is 0, the agent would check
    3. In this phase, the agent has 7 cards to consider and take actions. Depending on the current hand type, the agent
    makes appropriate decision to make best profit.
        --> even two pair or three of a kind has chance to win as the agent get a high hole card, so we can call
        with less amount at this time
        --> if straight, flush, or full house, call with amount no larger than
        --> if four of a kind, straight flush, or royal flush, all-in
        --> otherwise fold
    :param player:
    ...
    rank_number = get_hand_type(values, suits)
    current_gameboard['final_hand_type'] = rank_number
    hole_card_type, hole_card_suit = current_gameboard['hole_card_type_v2']

    if previous_bet <= 0 and check in allowable_actions:
        return check, params

    if rank_number >= 8 and all_in in allowable_actions:
        return all_in, params
    elif rank_number >= 5 and call in allowable_actions:
        if previous_bet <= 2 * minimum_bet and previous_bet <= player.current_cash:
            return call, params
    else:
        if previous_bet <= 1.5 * minimum_bet and previous_bet <= player.current_cash and \
                (hole_card_type == 'top_1' or hole_card_type == 'top_2'):
            return call, params
...
def make_computing_best_hand_moves(player, current_gameboard, allowable_actions, code):
    ...
    :param player: current player
    :param current_gameboard: current game board
    :param allowable_actions: allowable action in this phase
    :param code:
    :return:
    """
    # parameters for actions to take
    ...
    if current_gameboard['calculate_best_hand'] in allowable_actions:
        ...
        raise Exception  # agent must compute its best hand at showdown
...
def make_continue_game_moves(player, current_gameboard, allowable_actions, code):
    ...
    elif current_gameboard['cur_phase'] == 'turn_phase' or current_gameboard['cur_phase'] == 'river_phase':
    ...
    #         return fold, params
        ...
        if player.current_decision == 'check' and not previous_bet and check in allowable_actions:
            ...
            amount_to_match = previous_bet - player.bet_amount_each_round
            if current_gameboard['final_hand_type'] >= 5:
                if amount_to_match + player.bet_amount_each_round < 2 * minimum_bet and amount_to_match < player.current_cash:
                    return match_highest_bet_on_the_table, params
                elif fold in allowable_actions:
                    return fold, params
            else:
                if amount_to_match + player.bet_amount_each_round < 1.5 * minimum_bet and amount_to_match < player.current_cash:
                    return match_highest_bet_on_the_table, params
                elif fold in allowable_actions:
                    return fold, params
