...
from gym_open_poker.envs import OpenPokerEnv
import types
...
def _alter_step(self, action, game_start=False):
    ...
    if time.time() - self.game_elements['_timer_time'] > TIME_LIMIT:
        logger.debug(f'{self.player_name} surpassed the 10-second decision time limit, conflicting with the novelty introduced by Rule 1.')
        print('aaaa')
        return [self._get_obs(stopped=True), self._get_reward(), True, False, self._get_info(stopped=True)]
