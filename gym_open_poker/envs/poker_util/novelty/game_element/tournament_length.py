...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.game_element.tournament_length")


class TournamentLength(gym.Wrapper):
    """
    This novelty adjusts {max_game_limitation\}, meaning that the tournament will conclude at the end of
    game {max_game_limitation}.
    """

    def __init__(self, env, max_game_limitation=10):

        super().__init__(env)
        sys.modules["initialize_game_elements"].initialize_game_element = getattr(
            sys.modules[__name__], "_alter_initialize_game_element"
        )
        global MAX_GAME_LIMITATION
        MAX_GAME_LIMITATION = max_game_limitation
...
def _alter_initialize_game_element(player_decision_agents, customized_arg_dict, random_seed):
    ...
    logger.debug(f"Novelty! This tournament will be terminated after game {MAX_GAME_LIMITATION}")
