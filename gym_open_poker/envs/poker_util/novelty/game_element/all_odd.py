...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.game_element.all_odd")


class AllOdd(gym.Wrapper):
    """
    This novelty introduces a new hand called "all_odd."
    This hand is defined as the highest combination across all hand types.
    """
...
def _alter_initialize_game_element(player_decision_agents, customized_arg_dict, random_func):
    ...
    logger.debug("Novetly! The new hand type all_odd was created.")
...
def get_all_odd(total_hand):
    ...
    hand = []
    for num in sorted(values, key=lambda x: -number_rank[x]):
        if num % 2 == 1:
            hand.append(num)
