...
class BigBlindChange(gym.Wrapper):
    """
    This novelty adjusts the {big_small_blind_ratio}, where the big blind is set to the product
    of {big_small_blind_ratio} and the small blind.

    """

    def __init__(self, env, big_small_blind_ratio=3):

        super().__init__(env)
        sys.modules["initialize_game_elements"].initialize_game_element = getattr(
            sys.modules[__name__], "_alter_initialize_game_element"
        )
        global BIG_SMALL_BLIND_RATIO
...
def _alter_initialize_game_element(player_decision_agents, customized_arg_dict, random_func):
    ...
    # novelty!!!!
    game_elements["big_small_blind_ratio"] = BIG_SMALL_BLIND_RATIO
    game_elements["big_blind_amount"] = game_elements["small_blind_amount"] * game_elements["big_small_blind_ratio"]
    _big_blind_amount = game_elements["big_blind_amount"]
    logger.debug(f"Novelty! The big blind amount is modified to {_big_blind_amount}")
