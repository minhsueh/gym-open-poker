...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.game_element.big_blind_change")


class BigBetChange(gym.Wrapper):
    """
    This novelty adjusts the {big_small_bet_ratio}. Specifically, the bet and raise_bet amount in the turn and river rounds
    will be {big_small_bet_ratio} times greater than in the pre-flop and flop rounds.
    """

    def __init__(self, env, big_small_bet_ratio=3):

        super().__init__(env)
        sys.modules["initialize_game_elements"].initialize_game_element = getattr(
            sys.modules[__name__], "_alter_initialize_game_element"
        )
        global BIG_SMALL_BET_RATIO
        BIG_SMALL_BET_RATIO = big_small_bet_ratio
...
def _alter_initialize_game_element(player_decision_agents, customized_arg_dict, random_seed):
    ...
    # novelty!!!!
    game_elements["big_small_bet_ratio"] = BIG_SMALL_BET_RATIO
    game_elements["big_bet"] = game_elements["big_blind_amount"] * game_elements["big_small_bet_ratio"]  # in turn and river
    _big_bet_amount = game_elements["big_bet"]
    logger.debug(f"Novelty! The bet and raise amount in turn and river is modified to {_big_bet_amount}.")
