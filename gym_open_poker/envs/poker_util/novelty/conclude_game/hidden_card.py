...
import copy
import logging
from dealer import find_winner, assign_money_to_winners, print_player_info, get_player_rank_list, log_best_card, log_ranking
from action import Action
import time


logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.conclude_game.hidden_card")


class HiddenCard(gym.Wrapper):
    """
    This novelty adds a hidden card among the community cards, which affects hand combinations when determining the winner.
    """
...
def _alter_conclude_game(current_gameboard):
    ...
    # novelty!!!!
    # deal one more card to community cards
    logger.debug("Novelty! Dealing one more community...")
    current_gameboard["board"].deal_community_card_by_number(1)
    logger.debug("Novelty! New ranking:")
    log_best_card(current_gameboard)
    log_ranking(current_gameboard)
