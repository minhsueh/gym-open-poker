...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.conclude_game.incentive")


class Incentive(gym.Wrapper):
    """
    At the end of each game, every active player who has not lost will receive an incentive of {amount}.
    """

    def __init__(self, env, amount=-10):
...
def _alter_conclude_game(current_gameboard):
    ...
    for player in current_gameboard["players"]:
        if player.status != "lost":
            logger.debug(f"Novelty! {player.player_name} is active, so that it will be incentivized ${AMOUNT}.")
            player.current_cash += AMOUNT
