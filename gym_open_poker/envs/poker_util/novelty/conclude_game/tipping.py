...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.conclude_game.tipping")


class Tipping(gym.Wrapper):
    """
    The winner should tip the player at the dealer position {ratio} of the winning amount.
    """

    def __init__(self, env, ratio=0.1):
...
def _alter_conclude_game(current_gameboard):
    ...
    for pot_idx in range(len(current_gameboard["board"].pots_amount_list)):
        ...
        tip = round(money_amount / len(winners) * TIP_RATIO, 2)
