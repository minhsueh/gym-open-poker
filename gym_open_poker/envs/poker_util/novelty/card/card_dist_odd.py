...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.card.card_dist_odd")


class CardDistOdd(gym.Wrapper):
    """
    This novelty, named 'Card2', alters the card distribution by showing all highest odd cards first.
    """

    def __init__(self, env, prohibit_parity="even"):

        super().__init__(env)
        assert prohibit_parity in [
            "even",
            "odd",
        ], f"prohibit_parity should be in ['even', 'odd'], current value is {prohibit_parity}"
        ...
        global PROHIBIT_PARITY
        PROHIBIT_PARITY = prohibit_parity
...
def _alter_initialize_cards(game_elements):
    ...
    if PROHIBIT_PARITY == "even":
        nums = ["A", "K", "J", "9", "7", "5", "3", "Q", "10", "8", "6", "4", "2"]
    elif PROHIBIT_PARITY == "odd":
        nums = ["Q", "10", "8", "6", "4", "2", "A", "K", "J", "9", "7", "5", "3"]
    else:
        raise
    #
