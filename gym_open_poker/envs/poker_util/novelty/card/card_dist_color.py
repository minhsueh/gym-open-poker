...
logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.card.card_dist_color")


class CardDistColor(gym.Wrapper):
    """
    This novelty, named 'Card3', alters the card distribution by showing all red cards (heart and diamond) first.
    """

    def __init__(self, env, prohibit_color="red"):

        super().__init__(env)
        assert prohibit_color in [
            "red",
            "black",
        ], f"prohibit_color should be in ['red', 'black'], current value is {prohibit_color}"
        ...
        global PROHIBIT_COLOR
        PROHIBIT_COLOR = prohibit_color
...
def _alter_initialize_cards(game_elements):
    ...
    suits, colors, prohibit_suits, prohibit_colors = [], [], [], []
    for suit, color in zip(all_suits, all_colors):
        if color != PROHIBIT_COLOR:
            suits.append(suit)
            colors.append(color)
        else:
            prohibit_suits.append(suit)
            prohibit_colors.append(color)
    suits += prohibit_suits
    colors += prohibit_colors
