...
from gym_open_poker.envs.poker_util.agents import agent_aggresive

logger = logging.getLogger("gym_open_poker.envs.poker_util.logging_info.novelty.agent.agent_add_aggressive")


class AddAgentAggressive(gym.Wrapper):
    """
    This novelty introduces {add_player_number} more agent_aggresive in the tournament.
    ...
    def __init__(self, env, add_player_number=2):
        ...
        for p_idx in range(1, add_player_number + 1):
            new_player_methods = getattr(sys.modules[__name__], "agent_aggresive")
