...
class Environment2(gym.Wrapper):
    """
    This novelty is 'Environment2'.
    At the conclusion of each game, every active player who has not lost will be charged $10.
    """
...
def _alter_conclude_game(current_gameboard):
    ...
    for player in current_gameboard["players"]:
        if player.status != "lost":
            logger.debug(f"Novelty! {player.player_name} is active, so that it will be charged $10.")
