...
class TestProb(unittest.TestCase):
    def test_get_probability_four_of_a_kind(self):
            ...
            total_hand = []
            # 5 singles
            hand_value = [1, 2, 3, 4, 5]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(0), "")
            # 5 card, two pairs
            hand_value = [1, 1, 2, 2, 6]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(2/47/46), "")
            # 5 card, one pair
            hand_value = [9, 2, 2, 3, 4]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(1/47/46), "")
            # 5 card, three of a kind
            hand_value = [2, 2, 2, 3, 4]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(1/47), "")  
            # 5 card, four of a kind
            hand_value = [2, 2, 2, 2, 4]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(1), "")    
            # 6 card, three pair
            hand_value = [9, 9, 2, 2, 12, 12]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(0), "")   
            # 6 card, three of a kind
            hand_value = [9, 9, 9, 2, 12, 12]
            self.assertEqual(get_probability_four_of_a_kind(total_hand, suits, hand_value), format_float_precision(1/46), "") 
